...
# -*- coding: utf-8 -*-
"""
FDE 工程师面试 100 问 — Markdown → PDF 书籍生成器
...
OUTPUT_PDF = os.path.join(BASE_DIR, "FDE工程师面试100问.pdf")
...
def md_to_flowables(md_text, chapter_num=None):
    ...
    while i < len(lines):
        ...
        if stripped.startswith('# ') and not stripped.startswith('## '):
            ...
            if chapter_num is not None and (title_text.startswith(f'第{chapter_num}章') or re.match(r'^第.+章', title_text)):
...
def on_page_callback(canv, doc):
    ...
    if page_num >= 3:
        ...
        canv.drawString(25*mm, A4[1] - 12*mm, 'FDE 工程师面试 100 问')
        canv.drawRightString(A4[0] - 25*mm, A4[1] - 12*mm, 'AI Agent 实战指南')
        # 页眉线
...
# ── 9. 章节信息 ──
CHAPTERS = [
    (1, 'chapter_1.md',  '第一章：FDE 角色认知与职业定位'),
    (2, 'chapter_2.md',  '第二章：AI Agent 技术基础'),
    (3, 'chapter_3.md',  '第三章：客户需求挖掘与业务分析'),
    (4, 'chapter_4.md',  '第四章：数据工程与知识处理'),
    (5, 'chapter_5.md',  '第五章：方案设计与架构规划'),
    (6, 'chapter_6.md',  '第六章：快速开发与原型验证'),
    (7, 'chapter_7.md',  '第七章：私有化部署与系统集成'),
    (8, 'chapter_8.md',  '第八章：上线运维与持续迭代'),
    (9, 'chapter_9.md',  '第九章：安全合规与风险管理'),
    (10, 'chapter_10.md', '第十章：价值交付与客户成功'),
]
...
def build_cover():
    ...
    flowables.append(Paragraph('FDE 工程师面试 100 问', title_style))
    flowables.append(Spacer(1, 10*mm))
    flowables.append(Paragraph('AI Agent 时代的核心复合型交付岗位', subtitle_style))
...
def print_doc_structure():
    ...
    print("=" * 70)
    print()
    print("FDE工程师面试100问.pdf")
    print("├── 封面")
    print("│   └── FDE 工程师面试 100 问")
    print("│       └── AI Agent 时代的核心复合型交付岗位")
    print("├── 目录")
    for num, _, title in CHAPTERS:
        # 读取每章的 H2 标题作为子目录
...
def main():
    ...
    doc = BaseDocTemplate(
        ...
        topMargin=22*mm,
        bottomMargin=22*mm,
        title='FDE 工程师面试 100 问',
        author='AI Agent 实战指南',
        subject='FDE Forward Deployed Engineer Interview Guide',
    )
